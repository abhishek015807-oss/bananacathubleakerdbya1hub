...
local _lp = game:GetService("Players").LocalPlayer
	...
	local A = false
		...
		local localPlayer = Players.LocalPlayer
			...
			local selectTeam = (Settings and Settings["Select Team"] == "Pirate") and "Pirates" or "Marines"
				...
				local pg = localPlayer:FindFirstChild("PlayerGui")
					local main = pg:FindFirstChild("Main (minimal)") or pg:FindFirstChild("Main")
						local ct = main.ChooseTeam
							local container = ct:FindFirstChild("Container")
								local teamFolder = (selectTeam == "Pirates") and container:FindFirstChild("Pirates") or container:FindFirstChild("Marines")
...
local genv = getgenv()
genv.LPH_ATTRIBUTES = function() end
genv.LPH_NO_VIRTUALIZE = function() end
genv.LPH_JIT = function() end
genv.LPH_JIT_MAX = function() end
genv.LPH_NO_UPVALUES = function() end
genv.LPH_ENCSTR = function() end
genv.LPH_HOOK_FIX = function() end
genv.VM = function() end
genv.NONE = nil
local Settings = {}
local HttpService = game:GetService("HttpService")
local HttpService = HttpService
local FolderName = "Banana Cat Hub"
local SaveFileNameGame = "-BloxFruitBNNC.json"
local Players = game:GetService("Players")
local SaveFileName = (Players.LocalPlayer.Name .. -BloxFruitBNNC.json)
local SaveSettings = function()
    _v2[_1] = nil
    makefolder("Banana Cat Hub")
    local _v6 = HttpService:JSONEncode(_v2)
    writefile((Banana Cat Hub .. (/ .. SaveFileName)), _v6)
end
local _ = genv.Config
local ReadSetting = function()
    makefolder("Banana Cat Hub")
    local _v7 = readfile((Banana Cat Hub .. (/ .. SaveFileName)))
    local _v8 = HttpService:JSONDecode(_v7)
end
local _v = makefolder("Banana Cat Hub")
local _v1 = readfile((Banana Cat Hub .. (/ .. SaveFileName)))
local _v2 = HttpService:JSONDecode(_v1)
genv.Settings = _v2
local PrepareMultiSelectList = function() end
local EnsureAllTrueDefaults = function() end
local _v3 = game:IsLoaded()
local FireButton = function()
    _1.Selectable = true
    local GuiService = game:GetService("GuiService")
    GuiService.SelectedObject = _1
    local VirtualInputManager = game:GetService("VirtualInputManager")
    local _v9 = VirtualInputManager:SendKeyEvent(true, "Return", false, _1)
    local _v10 = VirtualInputManager:SendKeyEvent(false, "Return", false, _1)
    local onActivated = _1.Activated:Connect(function(...) end)
end
local _v = task.spawn(function(...)
    GuiService.SelectedObject = nil
end)
genv.ExploitReq = function()
    local _v11 = request(_1)
end
local _v4 = gethui()
local _v5 = _v4:FindFirstChild("Nousigi Hub GUI")
local _ = genv.__BF_RELOAD
_v5.Enabled = true
genv.UIToggled = true
